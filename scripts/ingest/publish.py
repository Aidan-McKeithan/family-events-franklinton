...
from scripts.ingest.data_model import validate_source

ROOT = Path(__file__).parents[2]
REGISTRY = ROOT / "scripts" / "ingest" / "source_registry.json"
...
def source_rows(payload, registry):
    ...
    unknown = set(by_name) - {item["sourceName"] for item in registry}
    if unknown:
...
def build_publish_sql(payload, registry=None, published_at=None, current_generated_at=None):
    ...
    statements.append("INSERT INTO catalog_metadata (id,generated_at,origin_label,origin_latitude,origin_longitude,updated_at) VALUES (1," + ",".join([sql_string(published_at), sql_string(origin.get("label", "Franklinton, NC 27525")), sql_number(origin.get("latitude", 36.101)), sql_number(origin.get("longitude", -78.458)), sql_string(published_at)]) + ") ON CONFLICT(id) DO UPDATE SET generated_at=excluded.generated_at,origin_label=excluded.origin_label,origin_latitude=excluded.origin_latitude,origin_longitude=excluded.origin_longitude,updated_at=excluded.updated_at;")
